...
2. Estimate the spatial distance between each person-vehicle pair.
...
2. **Alternative: Train from scratch on custom dataset.**
...
```
calibrations/
├── cam_01_homography.json
├── cam_02_homography.json
└── ...
```
...
| Person partially behind vehicle            | High       | High     | Tracking maintains ID through brief occlusion |
...
| False proximity from stacked goods         | Medium     | Medium   | Height-based filtering — detected objects below expected vehicle height are likely goods |
