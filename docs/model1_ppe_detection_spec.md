...
2. **Stage 2 — Full Fine-Tuning:** Unfreeze all layers. Train for 80–120 additional epochs with cosine annealing learning rate schedule. Initial LR: 1e-4, final LR: 1e-6.
3. **Stage 3 — Domain Adaptation:** Fine-tune for 10–15 epochs on site-specific data (captured from actual deployment cameras) with very low learning rate (1e-5).
...
2. **TensorRT Conversion:** Use `trtexec` or Ultralytics built-in TensorRT export with FP16 precision.
3. **Validation:** Compare ONNX and TensorRT outputs against PyTorch baseline — mAP drop must be < 0.5%.
...
|------------------------------------|------------|--------|-----------------------------------------------|
| Helmet misclassified as hard hat   | Medium     | Medium | Include diverse helmet types in dataset        |
| Vest color confused with clothing  | High       | High   | Train with diverse clothing colors, not just neon yellow |
| Small person at distance           | High       | Medium | Multi-scale training, smaller anchor sizes     |
| Occluded worker behind machinery   | High       | High   | Train with occlusion augmentation, track-based confirmation |
| Night / low-light conditions       | Medium     | High   | Low-light augmentation, IR camera support      |
| Camera angle variation             | Medium     | Medium | Include top-down and side-angle data           |
